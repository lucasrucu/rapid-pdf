...
| `ui/page_jump.py` | `PageJump`, the status-bar page box: the current page as an editable field with the total beside it. Type a number and press Enter (or `Ctrl+G` to get there) and the editor and the Organizer both go to it. Out-of-range numbers clamp, an abandoned edit snaps back, and it stops mirroring the scroll position while you are typing in it. |
| `ui/scrolling.py` | `TrackpadScrollFilter` and the wheel-delta helpers. A mouse wheel reports `angleDelta` in 120-unit notches, a precision trackpad reports `pixelDelta` in a stream of small events that Qt's own scroll handling ignores outright. Everything that scrolls or steps per notch reads the delta through here, so the mouse path is untouched and a trackpad moves by the pixels the finger actually travelled. |
| `ui/preferences_dialog.py` | `PreferencesDialog`, Edit > Preferences (`Ctrl+,`). One page of `QGroupBox`es over `core/settings.py`, with no OK and no Cancel: every control applies as it is touched, because the theme and page panel toggles already did that from the View menu and one setting cannot behave two ways. It holds no copy of any setting; the page panel checkbox drives the View menu's own `QAction`, the theme dropdown drives the `ThemeManager`, and the fit dropdown drives `MainWindow.choose_fit_mode`, which is the same entry point the status bar's fit icons use. |
