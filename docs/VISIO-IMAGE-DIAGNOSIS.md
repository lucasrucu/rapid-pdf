...
Detection, lift, click-to-grab, Shift+click (no lift, selection intact), double-click exclusion, and no-double-bake-across-saves all pass; `smoke_test.py` passes. GUI feel (the actual drag/click in the window) still needs hands-on testing, and that's what this branch is for.
