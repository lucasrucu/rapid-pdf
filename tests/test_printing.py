...
WHAT IS NOT TESTED, and is not claimed to be:
  ...
  - QPrintDialog and QPrintPreviewDialog opening. Both are modal and one of
    them is the Windows system dialog; a test that opens either hangs the
    suite. `print_active_document` is exercised with both patched out.
  ...
  - colour management, duplex, stapling, trays. None of it is ours: it belongs
    to the printer dialog and is carried on the QPrinter untouched.
"""
...
from ui.print_support import (
    ACTUAL_SIZE, FIT_PAGE, MAX_RENDER_MEGAPIXELS, PRINT_DPI_CAP,
    RANGE_ALL, RANGE_CURRENT, RANGE_CUSTOM, RANGE_SELECTION,
    PageRangeError, PrintOptions, PrintOptionsDialog, PrintResult,
    close_render, fit_page, markup_baked_copy, orientation_for,
    parse_page_range, print_active_document, print_document, render_zoom,
    resolve_pages, source_from_view,
)
...
def test_a_range_split_over_spaces_is_not_two_pages():
    """'1 - 3' has to mean 1 to 3, not pages 1 and 3."""
    assert parse_page_range("1 - 3", 10) == [0, 1, 2]


@pytest.mark.parametrize("text", [
    "", "   ", "abc", "1-abc", "1..3", "1-2-3", "-", " - ", "0", "0-3",
    "1,,x", "1/3", "+2", ",", "1 2 x", "--5", "3-4-", "1.5",
])
...
def test_a_trailing_comma_alone_is_not_an_error():
    """'1-3,' is a trailing separator, not a malformed token."""
    assert parse_page_range("1-3,", 10) == [0, 1, 2]
...
def test_a_range_that_straddles_the_end_keeps_what_exists():
    assert parse_page_range("8-30", 10) == [7, 8, 9]
...
def test_the_error_is_a_finished_sentence():
    """The dialog shows it verbatim, so it has to read like English."""
    with pytest.raises(PageRangeError) as caught:
...
def test_resolve_pages_covers_the_three_that_need_no_typing(mode, expected):
    assert resolve_pages(mode, 5, current_page=2,
                         selection=(3, 1)) == expected


def test_resolve_pages_sorts_and_filters_a_selection():
    """A selection arrives in click order and may name a page that has gone."""
    assert resolve_pages(RANGE_SELECTION, 3, selection=(2, 0, 9, 0)) == [0, 2]


def test_an_empty_selection_is_an_error_not_the_whole_document():
    with pytest.raises(PageRangeError):
        resolve_pages(RANGE_SELECTION, 5, selection=())
...
def test_fit_keeps_the_aspect_ratio_and_stays_inside_the_paper(page, paper):
    fit = fit_page(page[0], page[1], paper, FIT_PAGE, 300)
...
def test_fit_centres_what_is_left_over():
    ...
    fit = fit_page(*A4, paper, FIT_PAGE, 300)   # portrait page
    left = fit.dest.left()
...
def test_fit_grows_a_small_page_as_well_as_shrinking_a_big_one():
    ...
    paper = QRect(0, 0, 2480, 3508)
    fit = fit_page(420.0, 595.0, paper, FIT_PAGE, 300)
...
def test_an_a1_drawing_on_a4_paper_is_not_cropped():
    ...
    paper = QRect(0, 0, 2480, 3508)
    fit = fit_page(*A1_LANDSCAPE, paper, FIT_PAGE, 300)
    assert not fit.clipped
...
def test_actual_size_is_the_page_at_its_real_size():
    """A4 at 300 DPI is 8.27 x 11.69 inches, so 2481 x 3508 dots."""
    paper = QRect(0, 0, 2480, 3508)
    fit = fit_page(*A4, paper, ACTUAL_SIZE, 300)
    assert fit.scale == pytest.approx(300 / 72.0)
    assert fit.dest.width() == pytest.approx(595 * 300 / 72.0)
    assert fit.dest.height() == pytest.approx(842 * 300 / 72.0)


def test_actual_size_says_so_when_the_page_will_not_fit():
    paper = QRect(0, 0, 2480, 3508)                # A4 paper
    fit = fit_page(*A1_LANDSCAPE, paper, ACTUAL_SIZE, 300)
    assert fit.clipped
    assert fit.dest.width() / fit.dest.height() == pytest.approx(
        _aspect(A1_LANDSCAPE))


def test_actual_size_of_a_page_that_does_fit_is_not_reported_as_clipped():
    paper = QRect(0, 0, 3508, 4961)                # A3 paper at 300dpi
    fit = fit_page(*A4, paper, ACTUAL_SIZE, 300)
    assert not fit.clipped
...
def test_fit_offsets_by_the_target_origin():
    ...
    fit = fit_page(*A4, paper, FIT_PAGE, 300)
...
def test_fit_refuses_a_page_with_no_size(page_w, page_h):
    with pytest.raises(ValueError):
        fit_page(page_w, page_h, QRect(0, 0, 100, 100), FIT_PAGE, 300)


def test_fit_refuses_paper_with_no_printable_area():
    with pytest.raises(ValueError):
        fit_page(*A4, QRect(0, 0, 0, 0), FIT_PAGE, 300)


# ---------------------------------------------------------------------------
# 3. Orientation, per page
# ---------------------------------------------------------------------------
...
def test_render_zoom_caps_the_paper_resolution():
    ...
    fit = fit_page(*A4, paper, FIT_PAGE, 1200)
...
def test_render_zoom_leaves_a_low_resolution_printer_alone():
    ...
    fit = fit_page(*A4, paper, FIT_PAGE, 150)
...
def test_a_big_drawing_on_small_paper_costs_what_the_paper_costs():
    ...
    """
    paper = QRect(0, 0, 9916, 14033)                    # A4 at 1200 dpi
    fit = fit_page(*A1_LANDSCAPE, paper, FIT_PAGE, 1200)
...
def test_render_zoom_never_returns_zero():
    ...
    paper = QRect(0, 0, 1, 1)
    fit = fit_page(*A1_LANDSCAPE, paper, FIT_PAGE, 72)
    assert render_zoom(*A1_LANDSCAPE, fit, 72) > 0


# ---------------------------------------------------------------------------
# 5. The page loop: one page at a time, and cancellable
# ---------------------------------------------------------------------------
...
def test_the_loop_counts_pages_clipped_at_actual_size(qt_app, mixed_pdf,
                                                      tmp_path):
    """The A1 page in the middle cannot fit A4 at its true size, and says so."""
    doc = _open(mixed_pdf)
    printer = _pdf_printer(tmp_path / "out.pdf")
    try:
        result = print_document(doc, [0, 1, 2], printer,
                                PrintOptions(scale_mode=ACTUAL_SIZE,
                                             match_page_orientation=False))
    finally:
        doc.close()
    assert result.printed == 3
...
def test_fit_never_reports_a_clipped_page(qt_app, mixed_pdf, tmp_path):
    doc = _open(mixed_pdf)
    printer = _pdf_printer(tmp_path / "out.pdf")
    try:
        result = print_document(doc, [0, 1, 2], printer,
                                PrintOptions(scale_mode=FIT_PAGE))
    finally:
        doc.close()
...
def test_each_sheet_takes_the_orientation_of_its_own_page(qt_app, mixed_pdf,
    ...
    The whole reason a commissioning pack is printable in one go. Asserted on
    the file rather than on the option, because Qt has to carry a page layout
    change across newPage() for it to be true, and that is a property of Qt.
    """
    out = tmp_path / "printed.pdf"
    doc = _open(mixed_pdf)
    try:
        print_document(doc, [0, 1, 2], _pdf_printer(out),
                       PrintOptions(match_page_orientation=True))
    finally:
        doc.close()
...
def test_turning_orientation_matching_off_prints_one_way_round(qt_app,
                                                               mixed_pdf,
                                                               tmp_path):
    out = tmp_path / "printed.pdf"
    doc = _open(mixed_pdf)
    try:
        print_document(doc, [0, 1, 2], _pdf_printer(out),
                       PrintOptions(match_page_orientation=False))
    finally:
        doc.close()
    printed = fitz.open(str(out))
    try:
        shapes = {(round(p.rect.width), round(p.rect.height)) for p in printed}
    finally:
        printed.close()
    assert len(shapes) == 1, "the sheets should all be the same way round"
...
def test_a_fitted_page_is_centred_on_the_sheet(qt_app, tmp_path):
    """A landscape page on portrait paper leaves equal white above and below.
...
# ---------------------------------------------------------------------------
# 7. Unsaved markup reaches the paper
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 8. The window: the menu entry, the source, the dialog
# ---------------------------------------------------------------------------
...
def test_the_source_reads_the_document_the_page_and_the_selection(win):
...
# ---------------------------------------------------------------------------
# 9. The options dialog, without opening it
# ---------------------------------------------------------------------------

def _dialog(window, **kwargs):
    return PrintOptionsDialog(window, source_from_view(window.view), **kwargs)


def test_the_dialog_defaults_to_every_page_and_fit(win):
    dialog = _dialog(win)
    assert dialog.range_mode() == RANGE_ALL
    assert dialog.options().scale_mode == FIT_PAGE
    assert dialog.options().match_page_orientation is True


def test_selection_is_offered_only_when_several_pages_are_selected(win):
    """One selected page is the ordinary state of the strip, not a selection.

    The page being read is always highlighted, so offering "Selected pages" for
    it would sit next to "Current page" saying the same thing in other words.
    """
    assert not _dialog(win)._range_btns[RANGE_SELECTION].isEnabled()
    win.view._page_panel._list.selectAll()
    assert _dialog(win)._range_btns[RANGE_SELECTION].isEnabled()


def test_the_dialog_turns_a_range_into_pages_on_accept(win):
    dialog = _dialog(win, range_mode=RANGE_CUSTOM, custom="2-3")
    dialog._commit(preview=False)
    assert dialog.result() == PrintOptionsDialog.DialogCode.Accepted
    assert dialog.choice().pages == [1, 2]
    assert dialog.choice().preview is False


def test_a_bad_range_keeps_the_dialog_open_and_says_why(win):
    dialog = _dialog(win, range_mode=RANGE_CUSTOM, custom="banana")
    dialog._commit(preview=False)
    assert dialog.result() != PrintOptionsDialog.DialogCode.Accepted
    assert dialog._error.isVisible() or dialog._error.text()
    assert "banana" in dialog._error.text()


def test_the_preview_button_carries_the_same_choices(win):
    dialog = _dialog(win, range_mode=RANGE_CUSTOM, custom="1")
    dialog._commit(preview=True)
    assert dialog.choice().preview is True
    assert dialog.choice().pages == [0]


def test_actual_size_can_be_chosen(win):
    dialog = _dialog(win, options=PrintOptions(scale_mode=ACTUAL_SIZE,
                                               match_page_orientation=False))
    assert dialog.options().scale_mode == ACTUAL_SIZE
    assert dialog.options().match_page_orientation is False


def test_a_remembered_selection_mode_falls_back_when_nothing_is_selected(win):
    """The last print used Selection; this one has nothing ticked."""
    dialog = _dialog(win, range_mode=RANGE_SELECTION)
    assert dialog.range_mode() == RANGE_ALL


# ---------------------------------------------------------------------------
# 10. The whole gesture, with the two modal dialogs stubbed out
# ---------------------------------------------------------------------------

def test_cancelling_the_options_dialog_prints_nothing(win, monkeypatch):
    monkeypatch.setattr(PrintOptionsDialog, "exec",
                        lambda self: PrintOptionsDialog.DialogCode.Rejected)
...
def test_cancelling_the_printer_dialog_prints_nothing(win, monkeypatch):
    ...
    def accept_options(self):
        ...
        return PrintOptionsDialog.DialogCode.Accepted

    monkeypatch.setattr(PrintOptionsDialog, "exec", accept_options)
    monkeypatch.setattr(ps.QPrintDialog, "exec",
                        lambda self: ps.QDialog.DialogCode.Rejected)
    result = print_active_document(win)
    assert result.cancelled


def test_the_gesture_prints_and_remembers_the_choices(win, monkeypatch,
                                                      tmp_path):
    """Ctrl+P end to end: both dialogs stubbed, a real file at the other end."""
    import ui.print_support as ps
    out = tmp_path / "gesture.pdf"

    def accept_options(self):
        self._range_btns[RANGE_CUSTOM].setChecked(True)
        self._custom.setText("1-2")
        self._scale_btns[ACTUAL_SIZE].setChecked(True)
        self._commit(preview=False)
        return PrintOptionsDialog.DialogCode.Accepted

    def accept_printer(self):
        printer = self.printer()
        printer.setOutputFormat(QPrinter.OutputFormat.PdfFormat)
        printer.setOutputFileName(str(out))
        return ps.QDialog.DialogCode.Accepted

    monkeypatch.setattr(PrintOptionsDialog, "exec", accept_options)
    monkeypatch.setattr(ps.QPrintDialog, "exec", accept_printer)
    ...
    try:
        ...
        printed.close()
    # Remembered on the window for the next Ctrl+P.
    assert win._print_range == RANGE_CUSTOM
    assert win._print_custom == "1-2"
    assert win._print_options.scale_mode == ACTUAL_SIZE
...
def test_the_result_reads_as_one_status_line():
    ...
    assert "did not fit" in PrintResult(printed=3, clipped=1).message()
