...
def test_delete_one_page_and_undo(win):
    win.view._delete_pages([1])
    assert _letters(win) == "ACDE"
    assert _rows(win) == 4
    win.view._canvas.undo_stack.undo()
    assert _letters(win) == "ABCDE"
    assert _rows(win) == 5


def test_delete_a_multi_page_selection_in_one_action(win):
    win.view._delete_pages([0, 2, 4])
...
def test_redo_reapplies_the_delete(win):
    win.view._delete_pages([0, 2, 4])
    win.view._canvas.undo_stack.undo()
    win.view._canvas.undo_stack.redo()
    assert _letters(win) == "BD"
    assert _rows(win) == 2
...
def test_deleting_every_page_is_refused(win, monkeypatch):
    ...
    win.view._delete_pages([0, 1, 2, 3, 4])
...
def test_reorder_a_multi_page_selection_keeps_relative_order(win):
    ...
    win.view._reorder_pages(order, [1, 3])
...
def test_moved_pages_stay_selected_after_the_drag(win):
    ...
    win.view._reorder_pages(order, [0, 1])
    assert _letters(win) == "CDEAB"
    assert win.view._page_panel.selected_rows() == [3, 4]


def test_a_bad_permutation_is_dropped_and_the_strip_rebuilt(win):
    win.view._reorder_pages([0, 1, 2], [0])          # too short for five pages
    assert _letters(win) == "ABCDE"
    assert _rows(win) == 5
...
def test_delete_then_reorder_undo_one_step_at_a_time(win):
    win.view._delete_pages([1])
    assert _letters(win) == "ACDE"
    win.view._reorder_pages(move_rows(4, [3], 0), [3])
...
def test_the_editor_stays_on_a_real_page_after_a_delete(win):
    win.view._on_page_selected(4)
    win.view._delete_pages([4])
    assert win.view._current_page == win.view._doc.page_count() - 1
    assert 0 <= win.view._current_page < win.view._doc.page_count()


def test_page_edits_mark_the_document_unsaved(win):
    assert not win.view._dirty
    win.view._delete_pages([1])
    assert win.view._dirty
...
def test_saving_writes_what_the_strip_shows(win, tmp_path):
    ...
    win.view._reorder_pages(move_rows(4, [0], 4), [0])
    shown = _letters(win)
    assert shown == "CDEB"
    out = tmp_path / "out.pdf"
    assert win.view._doc.save(str(out))
